...
class KnapsackApp:
    ...
    def setup_input_tab(self):
        ...
        ttk.Button(buttons_frame, text="🗑️ Clear All", command=self.clear_items).pack(side=tk.LEFT, padx=5)
    ...
    def setup_viz_tab(self):
        ...
        # DP Table tab
        self.dp_frame = ttk.Frame(self.viz_notebook, style='Card.TFrame')
    ...
    def run_dp(self):
        ...
        def func():
            dp_table, selected, total_value = dynamic_programming_knapsack(self.items, capacity_int)
            return {'DP': {'selected': [(item, 1.0) for item in selected], 'value': total_value, 'dp_table': dp_table}}
    ...
    def run_all(self):
        ...
        def run():
            try:
                ...
                # Run DP
                dp_table, selected_dp, total_dp = dynamic_programming_knapsack(self.items, capacity_int)
                ...
                results = {
                    ...
                    'DP': {'selected': [(item, 1.0) for item in selected_dp], 'value': total_dp, 'dp_table': dp_table},
    ...
    def update_viz(self):
        ...
        if 'DP' in self.results and 'dp_table' in self.results['DP']:
            try:
                ...
                plot_dp_table(self.results['DP']['dp_table'], self.items, int(float(self.capacity_var.get())), ax)
